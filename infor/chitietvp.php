  ...
  <?php
      ...
      $sql_chitietvp = mysqli_query($con,"SELECT * FROM tbl_chitietvp WHERE chitietvp_id='$id'");
      $row_chitietvp = mysqli_fetch_array($sql_chitietvp);
   ?>
   <?php
   ...
   ?>
 		 <div class="content_price">
       			<div class="content_price-id">
       				<span><i class="ti-save-alt"></i> MS: #<?php echo $row_chitietvp['chitietvp_id']; ?></span>
       			</div>

       			<div class="content_price-prices">
       				<i class="ti-wallet"></i>
       				<span><?php echo $row_chitietvp['chitietvp_prices']; ?> VNĐ</span>
       			</div>
       		...
       		<div class="content_info">
       			<div class="content_info-hanhtinh">
       				<i class="ti-rocket"></i>
       				<span>Hành tinh: <?php echo $row_chitietvp['chitietvp_planet']; ?></span>
       			</div>

       			<div class="content_info-sever">
       				<i class="ti-shine"></i>
       				<span>Sever:<?php echo $row_chitietvp['chitietvp_sever']; ?> </span>
       			</div>

       			<div class="content_info-dangki">
       				<i class="ti-settings"></i>
       				<span>Loại Đồ: <?php echo $row_chitietvp['chitietvp_type']; ?></span>
       			</div>
			</div>
			<?php
				$user_vp = $row_chitietvp['chitietvp_user'];
				$sql_show = mysqli_query($con,"SELECT * FROM tbl_uploads WHERE uploads_user_of_type = '$user_vp' AND uploads_type = 'VẬT PHẨM'");
			...
			<?php }?>
			<?php
			if(isset($row_chitietvp['chitietvp_id'])) {

				if(isset($_POST['pay'])){
					if($row_login['customer_wallet'] >= $row_chitietvp['chitietvp_prices']) {
						$row_login['customer_wallet'] = $row_login['customer_wallet'] - $row_chitietvp['chitietvp_prices'];
						$walletNew = $row_login['customer_wallet'];
						$usersession = $_SESSION['login'];
						$deleteID = $row_chitietvp['chitietvp_id'];
						$uservp = $row_chitietvp['chitietvp_user'];
						$passvp = $row_chitietvp['chitietvp_pass'];
						$img_vp = "./uploads/" . $row_chitietvp['chitietvp_img'];
						$sql_update = mysqli_query($con,"UPDATE tbl_customer SET customer_wallet = '$walletNew' WHERE customer_user = '$usersession'");
						echo '<script>alert("Mua thành công, bạn vào lịch sử giao dịch để xem thông tin nhé!") ; window.location ="lsgd.php?id=3";</script>';
						$sql_lsgd = mysqli_query($con,"INSERT INTO tbl_lsgd(lsgd_userkh, lsgd_type, lsgd_tk, lsgd_mk) VALUE ('$usersession', 'VẬT PHẨM', '$uservp', '$passvp')");
						// $sql_delete  = mysqli_query($con,"DELETE FROM tbl_chitietvp WHERE chitietvp_id = '$deleteID'");
						// unlink($img_vp);
						// $user_vp = $row_chitietvp['chitietvp_user'];
						// $sql_show = mysqli_query($con,"SELECT * FROM tbl_uploads WHERE uploads_user_of_type = '$user_vp' AND uploads_type = 'VẬT PHẨM'");
						// while ($row_show = mysqli_fetch_array($sql_show)) {
						// 	$img_des = "./uploads/" . $row_show['uploads_img'];
						// 	$sql_delete_show  = mysqli_query($con,"DELETE FROM tbl_uploads WHERE uploads_user_of_type = '$user_vp' AND uploads_type ='VẬT PHẨM'");
						// 	unlink($img_des);
						// }
					}
					else {
			...
			<div class="modal__pay">
				...
				<form action="" method="post">
					...
					<div class="modal-info">
						<h4 class="modal-info-heading">Thông tin tài khoản #<?php echo $row_chitietvp['chitietvp_id']; ?></h4>
						<ul class="modal-info-list">
						
							<li class="modal-info-item modal-info-item-separate">Hành tinh :<?php echo $row_chitietvp['chitietvp_planet']; ?></li>
							<li class="modal-info-item modal-info-item-separate">Sever :<?php echo $row_chitietvp['chitietvp_sever']; ?></li>
							<li class="modal-info-item modal-info-item-separate">Giá : <?php echo number_format($row_chitietvp['chitietvp_prices'], 0, ' ',','); ?>VNĐ</li>
